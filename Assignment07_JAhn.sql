...
Begin Try
	Use Master;
	If Exists(Select Name From SysDatabases Where Name = 'Assignment07DB_JAhn')
	 Begin 
	  Alter Database [Assignment07DB_JAhn] set Single_user With Rollback Immediate;
	  Drop Database Assignment07DB_JAhn;
	 End
	Create Database Assignment07DB_JAhn;
End Try
Begin Catch
	Print Error_Number();
End Catch
go
Use Assignment07DB_JAhn;
...
AS
	SELECT Top 100000000000
		...
		[InventoryDate] = DateName(mm, InventoryDate) + ', '+ DateName(Year, InventoryDate),
		[InventoryCount] = COUNT,
		[PreviousMonthCount] = IIF(DatePart(Month, InventoryDate) = 1, 0, Lag(Sum(Count)) Over (Order By ProductName, Year(InventoryDate)))
	From vProducts as P
		Inner Join vInventories as I
			On P.ProductID = I.ProductID
		Group By ProductName, InventoryDate, Count
